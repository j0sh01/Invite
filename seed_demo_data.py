...
def seed_demo_data():
	...
	# Create 5 events
	wedding = _create_event("John & Mary's Wedding", "Wedding", "Completed", -30, "10:00", 
...
def _create_event(name, event_type, status, days_from_now, time_str, venue, organizer, contact, email, max_guests):
	...
	ev.currency = "TZS"
	ev.allow_partial_payments = 1
	ev.enable_reminders = 1
	ev.reminder_days_before = 3
	ev.description = f"A demo {event_type.lower()} event for testing purposes."
	ev.insert(ignore_permissions=True)
	print(f"   ✅ Created Event: {ev.event_name}")
	return ev.name
